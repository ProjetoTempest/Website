...
from app.schemas.product import ProductRequest, ProductUpdate, ProductResponse
...
class ProductCases:
    ...
    def add(self, product: ProductRequest) -> dict[str, str]:
        try:
            product_on_db = self.db_session.query(ProductModel).filter_by(title=product.title).first()
            
            if product_on_db:
                raise HTTPException(status_code=400, detail="Produto já cadastrado")

            product_on_db = ProductModel(**product.model_dump(exclude={"images"}), id=id_generate())
            ...
            list_imgs = [ImageProductModel(url=img_url, item_id=id, id=id_generate()) for img_url in product.images]
    ...
    def get(self, product_id: str) -> ProductResponse:
        try:
            ...
            product_response = self._assemble_product_response(product=product_db) 
    ...
    def get_all(self) -> list[ProductResponse]:
        try:
            product_list = self.db_session.query(ProductModel).options(joinedload(ProductModel.images)).all()
            
            if not product_list:
                raise HTTPException(status_code=404, detail="Não há nenhum produto cadastrado")
    ...
    def _product_model(self, product_title: str) -> ProductModel:
        product_db = self.db_session.query(ProductModel).filter_by(title=product_title).first()
    ...
    def _product_model_id(self, product_id: str) -> ProductModel:
        product_db = self.db_session.query(ProductModel).filter(ProductModel.id == product_id).options(joinedload(ProductModel.images)).first()
    ...
    def _map_models_to_responses(self, products: list[ProductModel]) -> list[ProductResponse]:
        return [self._assemble_product_response(product=product) for product in products]
 
    def _assemble_product_response(self, product: ProductModel) -> ProductResponse:
        if not product.images:
            return ProductResponse(**product.dict())
        return ProductResponse(**product.dict(), images=[ImagesProductsResponse(**img.dict()) for img in product.images])