...
from app.schemas.service import ServiceRequest, ServiceResponse, ServiceUpdate
from app.use_cases.service import ServiceCases
...
async def create_service(
    ...
    Args:
        service (ServiceRequest): Um objeto que contém os dados do produto a ser criado.
    ...
    return ServiceCases(db_session=db).add(service)
...
def read_service(service_id: str, db: Session = Depends(get_db)):
    ...
    Args:
        service_id (str): O ID do produto a ser recuperado.
        db (Session): Uma sessão do banco de dados injetada automaticamente pelo FastAPI.

    Returns:
        ServiceResponse: Um objeto que representa os detalhes do produto.

    Raises:
        HTTPException: Se o produto não for encontrado, retorna uma exceção HTTP 404.
    """
    return ServiceCases(db_session=db).get(service_id)


@router.get("/all/", status_code=status.HTTP_200_OK, response_model=list[ServiceResponse])
def read_services(db: Session = Depends(get_db)):
    ...
    Returns:
        list[ServiceResponse]: Uma lista de objetos que representam os produtos registrados.
    """

    return ServiceCases(db_session=db).get_all()


@router.put("/update/", status_code=status.HTTP_200_OK)
def update_service(service_update: ServiceUpdate, db: Session = Depends(get_db)):
    """
    ...
    Args:
        service_update (ServiceUpdate): Os dados atualizados do produto.
    ...
    return ServiceCases(db_session=db).update(service_update)


@router.delete("/delete/")
def delete_service(id: str, db: Session = Depends(get_db)):
    """
    ...
    return ServiceCases(db_session=db).delete(service_id=id)
